...
namespace Morito
{
    ...
    public class InputState
    {
        ...
        public float AmountForwardThruster(PlayerIndex? controllingPlayer, out PlayerIndex playerIndex)
        {
            ...
            {
                if (IsKeyDown((Keys)KeyboardControls.accelerateKey, controllingPlayer, out playerIndex))
                {
                    thrustAmount++;
                    MoritoFighterGame.MoritoFighterGameInstance.DisplayedMessages["keyup"] = "keyup is being pressed ";
                }
                
                /* Might remove this code if we don't want to move in the reverse (hint: remove the / at the end of this line) */
                if (IsKeyDown((Keys)KeyboardControls.deaccelerateKey, controllingPlayer, out playerIndex))
                    thrustAmount--;
                /* */
            }
        ...
        public int KeyboardLeftorRight(PlayerIndex? controllingPlayer, out PlayerIndex playerIndex)
        {
            if ( IsKeyDown((Keys)KeyboardControls.turnLeftKey, controllingPlayer, out playerIndex) )
                return 1;

            if ( IsKeyDown((Keys)KeyboardControls.turnRightKey, controllingPlayer, out playerIndex) )
                return -1;
