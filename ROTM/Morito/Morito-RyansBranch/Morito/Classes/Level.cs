...
namespace Morito
{
    public class Level
    {
        ...
        public void loadFromXElement(XElement root)
        {
            ...
            // Load weapons to spawn
            int totalWeapons = int.Parse(root.Element("maxNumWeapons").Value);
            List<Weapons> weapons = new List<Weapons>();
            foreach (XElement obj in root.Element("AvailableWeapons").Elements())
            {
                weapons.Add( (Weapons)int.Parse(obj.Value) );
            }

            // Choose a random weapon and create a floating weapon of it to add to the game
            Random rand = new Random(234731);
            FloatingWeapon weapon;
            int numWeapons = weapons.Count;
            int next;
            for (int i = 0; i != totalWeapons; ++i)
            {
                next = rand.Next(numWeapons);
                weapon = WeaponGenerator.generateFloatingWeapon(weapons[next], GameScreen);
                weapon.Position2D = new Vector2(rand.nextFloat() * 30, rand.nextFloat() * 30);
                weapon.Velocity = new Vector2(rand.nextFloat() / 10, rand.nextFloat() / 10);
