...
namespace Morito
{
    public class HumanPlayer: Player, isControllable
    {
        ...
        public override void Update(GameTime gameTime)
        {
            ...
            PlayerIndex dummy;
            force = Input.AmountForwardThruster(PlayersIndex, out dummy);
            MoritoFighterGame.MoritoFighterGameInstance.DisplayedMessages["update"] = "force: " + force;
            ...
            Vector2 direction = Input.AmountFaceRightUp(PlayersIndex, out dummy);

            // Key board controls for turning
            int keyboardDirection = Input.KeyboardLeftorRight(PlayersIndex, out dummy);
            if (keyboardDirection != 0)
            {
                PlayersShip.RotationAngle += (keyboardDirection * PlayersShip.RotationSpeed);
            }
            else
            {
                //check if the stick is in the dead zone of the controller
                if (!((Math.Abs(direction.X) < 0.010) &&
                       (Math.Abs(direction.Y) < 0.010)))
                {
                    //rotate the ship
                    PlayersShip.RotationAngle = TurnToFace(
                        PlayersShip.Position2D,
                        new Vector2(direction.X, direction.Y) * 100,    //hmm why not just direction (instead of new vector2)
            ...
            if (Input.IsShootingTopWeapon(PlayersIndex, out dummy))
                PlayersShip.FireWeapon(FireDirection.Up, gameTime);
            else if (Input.IsShootingRightWeapon(PlayersIndex, out dummy))
                PlayersShip.FireWeapon(FireDirection.Right, gameTime);
            else if (Input.IsShootingBottomWeapon(PlayersIndex, out dummy))
                PlayersShip.FireWeapon(FireDirection.Down, gameTime);
            else if (Input.IsShootingLeftWeapon(PlayersIndex, out dummy))
                PlayersShip.FireWeapon(FireDirection.Left, gameTime);
        }
       ...
       /*public void applyControlsGamePad(GameTime gameTime)
        {
            ...
            if (_currentGamePad.Buttons.Y == ButtonState.Pressed)
                PlayersShip.FireWeapon((Keys)KeyBoardControls.FireUp, gameTime);

            if (_currentGamePad.Buttons.A == ButtonState.Pressed)
                PlayersShip.FireWeapon((Keys)KeyBoardControls.FireDown, gameTime);

            if (_currentGamePad.Buttons.B == ButtonState.Pressed)
                PlayersShip.FireWeapon((Keys)KeyBoardControls.FireRight, gameTime);

            if (_currentGamePad.Buttons.X == ButtonState.Pressed)
                PlayersShip.FireWeapon((Keys)KeyBoardControls.FireLeft, gameTime);
        }

        public void applyControlsKeyboard(GameTime gameTime)
        {
            ...
            if (_currentKeyboard.IsKeyDown((Keys)KeyBoardControls.Up))
                force++;

            if (_currentKeyboard.IsKeyDown((Keys)KeyBoardControls.Down))
                force--;
            ...
            if (_currentKeyboard.IsKeyDown((Keys)KeyBoardControls.Left))
                PlayersShip.RotationAngle += PlayersShip.RotationSpeed;

            if (_currentKeyboard.IsKeyDown((Keys)KeyBoardControls.Right))
                PlayersShip.RotationAngle -= PlayersShip.RotationSpeed;

            if ( _currentKeyboard.IsKeyDown((Keys)KeyBoardControls.FireUp) )
                PlayersShip.FireWeapon((Keys)KeyBoardControls.FireUp, gameTime);

            if ( _currentKeyboard.IsKeyDown((Keys)KeyBoardControls.FireDown) )
                PlayersShip.FireWeapon((Keys)KeyBoardControls.FireDown, gameTime);

            if ( _currentKeyboard.IsKeyDown((Keys)KeyBoardControls.FireRight) )
                PlayersShip.FireWeapon((Keys)KeyBoardControls.FireRight, gameTime);

            if ( _currentKeyboard.IsKeyDown((Keys)KeyBoardControls.FireLeft) )
                PlayersShip.FireWeapon((Keys)KeyBoardControls.FireLeft, gameTime);
        }*/
        /*public override void Update(GameTime gameTime)
        {
            ...
            {
                applyControlsGamePad(gameTime);
            }
            //if no controller is supported use the keyboard for input
            else
            {
                applyControlsKeyboard(gameTime);
            }
        }*/
    }
}
